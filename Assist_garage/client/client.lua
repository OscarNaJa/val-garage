...
checkCanDeposit = function(plate,vehicle,id) 
    for _ , v in pairs(Mystored) do 
        ...
        if v.plate == plate and vehiclemodel.model == vehicle.model and v.deposit == nil then 
...
checkOwner = function(plate,model)
    for _ , v in pairs(Mystored) do
        local vehiclemodel = json.decode(v.vehicle).model 
        if v.plate == plate and (vehiclemodel == model or GetDisplayNameFromVehicleModel(model):lower() == vehiclemodel) then 
            v.stored = true 
            return true 
        end 
    end 
...
removeDeposit = function(plate)
    ...
    for _ , v in pairs(Mystored) do
        if v.plate == plate and v.deposit ~= nil then 
            local num = v.deposit
...
function SpawnVehicle(vehicle, plate, damage)
    ...
    local myDim = exports['Assist_Setdimen']:GetDimension()
    if myDim ~= 0 and not isWhitelistedDimension(myDim) then
        exports['Assist_Setdimen']:SetDimension(0)
        Wait(500)
    end
    -- while exports['Assist_Setdimen']:GetDimension() ~= 0 do
    --     exports['Assist_Setdimen']:SetDimension(0)
    --     Wait(100)
    -- end
...
RegisterNUICallback('spawnvehicle', function(data,cb)
    ...
    if tableData then 
        if delayCheck then return end 
        local damage = json.decode(tableData.health_vehicles)
        -- print('CurrentPoint '..tostring(CurrentPoint))
            ...
            ESX.TriggerServerCallback(ResourceName..':payMoney', function(hasEnoughMoney)
                if hasEnoughMoney then
                    ...
                    TriggerEvent("mythic_progbar:client:progress", {
                        ...
                        openuigarage = false
                        if not status then
                            -- print('Spawn vehicle from pound '..CurrentPoint)
                            CurrentPoint = nil
                            -- * optional หมายถึงจะใส่หรือไม่ใส่ก็ได้
...
function SaveDamage(vehicle, vehicleProps)
    ...
    for _ , v in pairs(Mystored) do 
        local vehiclemodel = json.decode(v.vehicle).model 
        if v.plate == vehicleProps.plate and (vehiclemodel == vehicleProps.model or GetDisplayNameFromVehicleModel(vehicleProps.model):lower() == vehiclemodel) then 
